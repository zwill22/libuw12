...
namespace uw12::linalg {
    ...
    inline Vec vec(const std::vector<double> &vector) {
        ...
        return vector;
#elif USE_EIGEN
        auto ptr = const_cast<double *>(vector.data());
    ...
    inline Mat mat(double *mem, const size_t n_row, const size_t n_col,
...
#elif USE_EIGEN
        if (!copy_data) {
            return Eigen::Map<Mat>(const_cast<double *>(mat.col(col1).data()), n_row,
                                   n_col);
        } else {
    ...
    inline bool nearly_equal(const Mat &mat1, const Mat &mat2,
