...
std::vector<linalg::Mat> calculate_integrals_direct(
  ...
  const auto parallel_fn =
      [&out, &offsets, &transform_fn, &integral_fn](const size_t A) {
        ...
        for (size_t sigma = 0; sigma < out.size(); ++sigma) {
          linalg::assign_cols(
              out[sigma], transform_fn(shell_results, sigma), off_a
          );
        }
      };
...
std::vector<linalg::Mat> calculate_X3idx_one_trans_direct(
    const BaseIntegrals &base_integrals, const utils::Orbitals &orbitals
) {
  ...
  const auto transform_fn =
      [&orbitals](const linalg::Mat &matrix, const size_t sigma) {
        return transformations::mo_transform_one_index_full(
            matrix, orbitals[sigma]
        );
      };
  ...
  return calculate_integrals_direct(
      base_integrals, integral_fn, transform_fn, n_rows
  );
}

std::vector<linalg::Mat> calculate_X3idx_one_trans_direct_ri(
    const BaseIntegrals &base_integrals, const utils::Orbitals &active_orbitals
) {
  const auto integral_fn = [&base_integrals](const size_t A) {
  ...
  const auto transform_fn =
      [&active_orbitals](const linalg::Mat &matrix, const size_t sigma) {
        return transformations::transform_second_index(
            matrix, active_orbitals[sigma]
        );
      };
...
std::vector<linalg::Mat> calculate_X3idx_two_trans_direct(
  ...
  const auto n_rows = [&occ_orbitals, &active_orbitals] {
    ...
    for (size_t i = 0; i < occ_orbitals.size(); ++i) {
      out[i] =
          linalg::n_cols(occ_orbitals[i]) * linalg::n_cols(active_orbitals[i]);
    }
    return out;
  }();
...
std::vector<linalg::Vec> calculate_X_D_direct(
    const BaseIntegrals &base_integrals, const utils::DensityMatrix &D
) {
  ...
  const auto parallel_fn = [&out, &base_integrals, &offsets, &density](
                               const size_t A
                           ) {
    const auto off_a = offsets[A];
    ...
    for (size_t sigma = 0; sigma < out.size(); ++sigma) {
      linalg::assign_rows(
          out[sigma], linalg::transpose(shell_results) * density[sigma], off_a
      );
    }
  };
  ...
  for (auto &integrals : out) {
    integrals = 0.5 * static_cast<double>(n_spin) *
                linalg::transpose(base_integrals.get_P2()) * integrals;
  }
  ...
  if (_occ_orbitals.size() == 1 || _occ_orbitals.size() == 2) {
    if (_occ_orbitals.size() != _active_orbitals.size()) {
      throw std::runtime_error(
          "different number of spin channels in occupied and active orbitals"
      );
    }
  } else {
  ...
  if (one_trans_ri->empty()) {
    ...
    parallel::isolate([&] {
      if (base_integrals->has_J3_ri_0()) {
        ...
        out = calculate_X3idx_one_trans_direct_ri(
            *base_integrals, active_orbitals
        );
      }
    });
      ...
      if (!one_trans->empty() || store_one_trans) {
        ...
        for (size_t sigma = 0; sigma < n_spin; ++sigma) {
          out[sigma] = transformations::mo_transform_two_index_full(
                           base_integrals->get_J3_0(),
                           active_orbitals[sigma],
                           occ_orbitals[sigma]
                       ) *
                       base_integrals->get_P2();
        }
        ...
        for (size_t sigma = 0; sigma < n_spin; ++sigma) {
          out[sigma] = transformations::mo_transform_two_index_full(
              base_integrals->get_J3(),
              active_orbitals[sigma],
              occ_orbitals[sigma]
          );
        }
      } else {
        out = calculate_X3idx_two_trans_direct(
            *base_integrals, occ_orbitals, active_orbitals
        );
      }
    });
...
std::vector<linalg::Vec> Integrals::get_X_D() const {
  ...
  if (base_integrals->has_J3_0()) {
    ...
    for (size_t sigma = 0; sigma < n_spin; ++sigma) {
      ...
      result[sigma] = 0.5 * static_cast<double>(n_spin) *
                      linalg::transpose(P2) * linalg::transpose(WV3_0) *
                      D_sigma;
    }
    ...
    for (size_t sigma = 0; sigma < n_spin; ++sigma) {
      ...
      result[sigma] =
          0.5 * static_cast<double>(n_spin) * linalg::transpose(WV3) * D_sigma;
    }
  } else {
