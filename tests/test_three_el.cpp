...
using uw12_test::density::random_density_matrix;
...
auto get_integral_fn() {
  return [](const std::vector<size_t>& n_occ,
            ...
            const int seed) {
    return uw12_test::setup_integrals_pair(
        n_ao, n_df, n_ri, n_occ, n_active, seed
    );
  };
}
...
void run_os_tests(
  ...
  return uw12_test::run_os_tests(W, V, fock_fn);
}
...
void run_ss_tests(
  ...
  return uw12_test::run_ss_test(W, V, fock_fn);
}
...
void run_test_full(
  ...
  const auto fock_fn = get_fock_fn(abs_projectors);
  return uw12_test::run_test_full(W, V, fock_fn);
}
...
TEST_CASE(
  ...
  const auto W_base = uw12_test::setup_base_integrals(n_ao, n_df, n_ri, W_seed);
  const auto V_base = uw12_test::setup_base_integrals(n_ao, n_df, n_ri, V_seed);
  ...
  uw12_test::check_equality_open_closed_spin(
      W_base, V_base, Co, active_Co, fock_fn
  );
}
...
TEST_CASE("Test three electron term - Closed Shell (Check cases)") {
  ...
  uw12_test::check_closed_shell_cases(integral_fn, fock_fn, fock0);
}
...
TEST_CASE("Test three electron term - Open Shell (No active orbitals)") {
  ...
  uw12_test::check_open_shell_no_active_orbitals(fock_fn, integral_fn, fock0);
}
...
void test_three_el_fock_all_electron(
  ...
  uw12_test::test_multi_el_fock_all_electron(
      fock_fn, W_base, V_base, D, threshold
  );
}
...
TEST_CASE("Test three electron term - Test Fock matrix (Closed Shell)") {
  ...
  const auto D = random_density_matrix(n_occ, n_ao, uw12_test::seed);
