...
#include "catch.hpp"
#include "integrals/transformations.hpp"
...
TEST_CASE("Test integrals - MO transformations") {
  ...
  for (size_t col_idx = 0; col_idx < n_df; ++col_idx) {
    ...
    const linalg::Mat mat2 = linalg::transpose(Co) *
                             linalg::reshape_col(J3_mjA, col_idx, n_ao, n_orb);
...
TEST_CASE("Test integrals - MO transformations (RI)") {
  ...
  for (size_t col_idx = 0; col_idx < n_df; ++col_idx) {
    ...
    const linalg::Mat mat2 =
        linalg::reshape_col(J3_pvA, col_idx, n_ri, n_ao) * C;
