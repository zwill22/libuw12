...
void run_test(const uw12_test::TestData & test_data, const double epsilon) {
  ...
  CHECK_THAT(
      energy, Catch::Matchers::WithinAbs(test_data.fock.energy, epsilon)
  );

  REQUIRE(uw12::utils::spin_channels(fock) == n_spin);
  for (size_t sigma = 0; sigma < n_spin; ++sigma) {
    CHECK(uw12::linalg::nearly_equal(fock[sigma], test_data.fock.fock[sigma], epsilon));
  }
}
...
TEST_CASE("Regression Test UW12 - Test Water") {
  ...
  run_test(test_data, 100 * test::epsilon);
}
